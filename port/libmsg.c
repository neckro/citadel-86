...
char getMessage(int (*Source)(void), char FromNet, char all, char ClearOthers)
{
	...
	c = (*Source)();
	switch (c) {
	case 'A': getMsgStr(Source, msgBuf.mbauth,  sizeof msgBuf.mbauth);    break;
	case 'D': getMsgStr(Source, msgBuf.mbdate,  NAMESIZE);    break;
	case 'C': getMsgStr(Source, msgBuf.mbtime,  NAMESIZE);    break;
	case 'H': getMsgStr(Source, msgBuf.mbMsgStat,  NAMESIZE);    break;
	case 'M': /* just exit -- we'll read off disk */  break;
	case 'N': getMsgStr(Source, msgBuf.mboname, NAMESIZE);
		  while ((d = strchr(msgBuf.mboname, '_')) != NULL)
			*d = ' ';
		  break;
	case 'O': getMsgStr(Source, msgBuf.mborig,  NAMESIZE);    break;
	case 'R': getMsgStr(Source, msgBuf.mbroom,  NAMESIZE);    break;
	case 'S': getMsgStr(Source, msgBuf.mbsrcId, NAMESIZE);    break;
	case 'T': getMsgStr(Source, msgBuf.mbto, sizeof msgBuf.mbto); break;
	case 'Q': getMsgStr(Source, msgBuf.mbaddr,  sizeof msgBuf.mbaddr);
						break;
	case 'X': getMsgStr(Source, msgBuf.mbdomain,NAMESIZE);    break;
	case 'P': getMsgStr(Source, msgBuf.mbOther, O_NET_PATH_SIZE); 
		  break;
	case 'W': getMsgStr(Source, CC, CC_SIZE);
		  if (ClearOthers)
	...
	case 't': getMsgStr(Source, msgBuf.mbaddr, sizeof msgBuf.mbaddr);
									break;
	case -1 : return FALSE;
	default:
	    if (isprint(c)) {
